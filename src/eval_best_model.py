...
import random
import shelve
from finalCombo import create_model,get_tokenizer,to_seq,pad_tag,prepare_input


db_file = "../models/models_db"
model_key = "combo_param_list"

DATASET_TEST = "../data/dataForEmbeddings_TEST.p"
...
def eval_model(model,dev):
    ...
    for index, row in df_data.tail(len(arr)).iterrows():
    	...
    	i+=1
    df_data.to_pickle("../predictions_dump/prediction_verA_DEV.p")
    os.system('python Scorer.py A_DEV')
    with open("../scorings/A_DEV.scores") as myfile:
        firstlines=myfile.readlines()[0:5] #put here the interval you want
    return float(firstlines[1].split(':')[1])


def eval_model_TEST(model,dev):
    """
    Return the MAP value, calling Scoring and reading the genereted files
    :param model: a trained model
    :param model: dev which we would like to have the MAP value (could be also for validation)
    ...
    for index, row in df_data.tail(len(arr)).iterrows():
    	...
    	df_data.set_value(index, 'dist_prediction', predicted_val[i][0])
    	i+=1
    df_data.to_pickle("../predictions_dump/prediction_verA_TEST.p")
    os.system('python Scorer.py A_TEST')
    #with open("../scorings/A_TEST.scores") as myfile:
    #    firstlines=myfile.readlines()[0:5] #put here the interval you want
    #return float(firstlines[1].split(':')[1])
...
if __name__=='__main__':
    ...
    if LOADMODEL:
        ...
        file_model="../models/final_combo.json"
        file_weight="../models/final_combo.h5"
