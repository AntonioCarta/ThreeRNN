...
def prepare_input(data):
    ...
    features = ['Comment_number', 'Has_Question',
       ...
       'Len_Comments_abs', 'Len_Comments_norm', 'Same_User','Relevance']

    binary = {'Irrelevant': [0.,1.], 'Relevant': [1.,0.], 'PerfectMatch': [1.,0.],'?':[0,0]}
...
def create_model(VOCAB, USE_GLOVE, EMBED_HIDDEN_SIZE, TRAIN_EMBED, SENT_HIDDEN_SIZE,
    ...
    feat = Input(shape=(11,), dtype='float32')
...
def eval_model(model,dev):
    ...
    for index, row in df_data.tail(len(arr)).iterrows():
    	...
    	i+=1
    df_data.to_pickle("../predictions_dump/prediction_verB_TASKB.p")
    os.system('python Scorer.py B_TASKB')
    with open("../scorings/B_TASKB.scores") as myfile:
...
if __name__=='__main__':
    ...
    while True:
        ...
        model.fit([training[0], training[1], training[2],training[3], training[4], training[5], training[6]], training[7],
                  batch_size=BATCH_SIZE, nb_epoch=randint(2,50))#, callbacks=[es], validation_split=0.1)
        ...
        if best_map < MAP:
            model.save_weights("../models/final_comboTASKB.h5", overwrite=True)
            with open("../models/final_comboTASKB.json", "w") as outfile:
